function agregarImagenes() {
    fetch(
            ...
            if (response.ok) {
                response.json().then((data) => {
                    ...
                    for (let i = 0; i < data.images.length; i++) {
                        imagenes += `<button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="${i}" ${i === 0 ? 'class="active"' : ""
        ...
        .catch((error) => {
            console.log(
                "Hubo un problema con la petición Fetch (agregar imagenes):" +
                error.message
            );
        });
}
...
function agregarComentarios() {
    fetch(
        ...
        .then((response) => {
            if (response.ok) {
                response.json().then((data) => {
                    ...
                    data.forEach((comentario) => {
                        const cal = document
                            .getElementsByClassName("calificaciones")
        ...
        .catch((error) => {
            console.log(
                "Hubo un problema con la petición Fetch (agregar comentarios):" +
                error.message
            );
        });
}
function relatedProducts() {
    fetch(
        ...
        .catch((error) => {
            console.log(
                "Hubo un problema con la petición Fetch (agregar productos relacionados):" +
                error.message
            );
        });
}
...
function agregarProducto(product, categoria) {
    ...
    <div class=" col-sm-12 id="container-product2">
        <div class="container col-md-4 col-sm-4" id="product-information">
            ...
            <button id="btn-agregar" class="CartBtn" data-product="${JSON.stringify(
        product
    )}">
    ...
    relatedProducts();
    document
        .getElementById("boton-enviar-datos")
        ...
        <div class="test-divs" style="border: 1px solid black; margin: 0px;  border: 0.5px solid #897d7d38;">
            <div style="display: flex; flex-direction: row;">
                <p class="test"><strong>${user.nombre + " " + user.apellido
                }</strong></p>
                <p style="margin: 0px 10px 0px 10px;">-</p>
                <p>${año}-${mes < 10 ? "0" + mes : mes}-${dia < 10 ? "0" + dia : dia
        ...
        `;
            ...
            const estrellitas = document
                .getElementsByClassName("calificaciones")
            [
                document.getElementsByClassName("calificaciones").length - 1
    ...
    .then((response) => {
        if (response.ok) {
            response.json().then((data) => {
                ...
                let i = 0;
                while (
                    data.products.length !== i &&
                    data.products[i].id !== parseInt(localStorage.ProductID)
                ) {
                    //console.log(i);
